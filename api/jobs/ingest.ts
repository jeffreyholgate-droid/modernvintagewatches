...
export default async function handler(req: any, res: any) {
  ...
  for (const cat of Object.values(Category)) {
    const priceMin = Number(
      overrides?.priceMin?.[cat] ?? settings.priceMin[cat as Category]
    );
    const priceMax = Number(
      overrides?.priceMax?.[cat] ?? settings.priceMax[cat as Category]
    );
   ...
   await log('INFO', `INGEST DEBUG: ${cat} raw=${raw.length}`);
    ...
    const filtered = raw.filter((i) => {
      ...
      const blocked = settings.blockKeywords
        ...
        .filter(Boolean)
        .some((k: string) => title.includes(k));
  ...
  await log(
    'INFO',
    `INGEST: complete; candidates=${all.length}; upserted=${count}`
  );
